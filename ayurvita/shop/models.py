...
class Product(models.Model):
    ...
    def get_offer(self):
        ...
        try:
            try:
                product_discount = ProductOffer.objects.get(product=self.id,is_active=True)
                offer=True
            except:
                category_discount = CategoryOffer.objects.get(category=self.category,is_active=True)
                offer=True
    ...
    def get_mrp(self):
            ...
            if product_discount:
                ...
                if category_discount:                 
                    if category_discount.discount > product_discount.discount:
                        mrp = self.product_price-self.product_price*(category_discount.discount/100)
                        self.mrp=mrp
                        return mrp
                    else:
                        mrp = self.product_price-self.product_price*(product_discount.discount/100)
                        self.mrp=mrp
                        return mrp
                else:
                    mrp = self.product_price-self.product_price*(product_discount.discount/100)
